...
use Stax\Builder\Core\Components\Abstract_Component;
use Stax\Builder\Core\Components\Nav;
use Stax\Builder\Core\Css_Generator;
use Stax\Customizer\Core\Controls\React\Instructions_Section;
...
abstract class Abstract_Builder implements Builder {
	...
	public function __construct() {
		...
		$this->set_property( 'layout_control_id', $this->control_id . '_v2' );
	...
	public function define_builder_settings() {
		SettingsManager::get_instance()->add(
			[
				...
				'default'            => '',
				'label'              => '',
				'type'               => '\Stax\Customizer\Core\Controls\React\Builder',
				'options'            => [
	...
	public function define_row_settings( $row_id ) {
		...
		if ( 'sidebar' !== $row_id ) {
			SettingsManager::get_instance()->add(
				[
					...
					'options'            => [
						'choices' => [
							'layout-full-contained' => __( 'Full Width', 'stax' ) . ' - ' . __( 'Contained', 'stax' ),
							'layout-fullwidth'      => __( 'Full Width', 'stax' ),
							'layout-contained'      => __( 'Contained', 'stax' ),
						],
					],
					'conditional_header' => $this->get_id() === 'header',
		...
		SettingsManager::get_instance()->add(
			[
				...
				'tab'                   => SettingsManager::TAB_STYLE,
				'section'               => $row_setting_id,
				'label'                 => __( 'Row Background', 'stax' ),
				'type'                  => 'stax_background_control',
				'live_refresh_selector' => 'sidebar' === $row_id ? $row_class . ' .header-menu-sidebar-bg' : $row_class,
				'live_refresh_css_prop' => [
					'cssVar'  => [
						'vars'     => 'backgroundControl',
						'selector' => 'sidebar' === $row_id ? '.header-menu-sidebar-bg' : '.' . $this->get_id() . '-' . $row_id,
					],
					'partial' => 'sidebar' === $row_id ? 'hfg_header_layout_partial' : $row_setting_id . '_partial',
				],
				'options'               => [
					'priority' => 100,
				],
				...
				'transport'             => 'postMessage',
				'sanitize_callback'     => [ Sanitizer::instance(), 'sanitize_background' ],
				'conditional_header'    => $this->get_id() === 'header',
			]
		);
	...
	protected function inline_builder_styles() {
		...
		if ( ! empty( $rows ) ) {
			foreach ( $rows as $row_id => $row_label ) {
				$style_array[ '#accordion-section-' . $this->control_id . '_' . $row_id ] = [
	...
	public function render() {
		...
		if ( is_customize_preview() ) {
			...
			$generator->set( $this->add_style() );

			echo '<style type="text/css" id="' . esc_attr( $this->get_id() ) . '-style">' . $generator->generate() . '</style>';// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	...
	private function get_row_featured_image( $image_url, $use_featured, $meta ) {
		...
		if ( ! empty( $use_featured ) && true === $use_featured && is_singular() ) {
		 ...
		 * @since 3.0.5
		 */
		return apply_filters( 'stx_builder_row_image_url', $image, $use_featured, $meta );
	}
	 ...
	 * @access  private
	 */
	private function add_row_style( $row_index, $css_array = [] ) {
		$css_array = $this->add_new_builder_styles( $css_array, $row_index );

		$rules          = [];
		$selector       = 'sidebar' === $row_index ? '.header-menu-sidebar-bg' : '.' . $this->get_id() . '-' . $row_index;
		...
		$rules['--rowBColor'] = [
			...
			Dynamic_Selector::META_DEFAULT => 'var(--stx-light-bg)',
		];
		...
		$default_color = isset( $default_colors['background'] ) ? $default_colors['background'] : 'var(--stx-site-bg)';
		...
		if ( 'color' === $background['type'] && ! empty( $background['colorValue'] ) ) {
			$rules = array_merge(
				$rules,
				[
					'--bgColor' => [
	...
	public function register_component( $component_to_add ) {
		if ( ! class_exists( $component_to_add ) || ! in_array( 'Stax\Builder\Core\Interfaces\Component', class_implements( $component_to_add ), true ) ) {
			return false;
		}
	...
	public function get_component( $id = null ) {
		if ( null === $id ) {
			$id = null === self::$current_component ? Abstract_Component::$current_component : self::$current_component;
		}
	...
	private function get_default_row_colors( $row_id ) {
		$bg_color_map = [
			'background' => [
				'dark-mode'  => 'var(--stx-dark-bg)',
				'light-mode' => 'var(--stx-site-bg)',
			],
			'text'       => [
				'dark-mode'  => 'var(--stx-text-dark-bg)',
				'light-mode' => 'var(--stx-text-color)',
			],
		];

		$is_footer_bottom = $this->get_id() === 'footer' && 'bottom' === $row_id;

		// On the new skin, the bottom footer row should be dark by default.
		if ( $is_footer_bottom ) {
			$bg_color_map = [
				'background' => [
					'dark-mode'  => 'var(--stx-site-bg)',
					'light-mode' => 'var(--stx-dark-bg)',
				],
				'text'       => [
					'dark-mode'  => 'var(--stx-text-color)',
					'light-mode' => 'var(--stx-text-dark-bg)',
				],
			];
		}
	...
	private function add_sidebar_controls( $row_setting_id ) {
		...
		SettingsManager::get_instance()->add(
			[
				...
				'options'            => [
					'choices' => [
						'slide_left'  => __( 'Slide from Left', 'stax' ),
						'slide_right' => __( 'Slide from Right', 'stax' ),
						'pull_left'   => __( 'Pull from Left', 'stax' ),
						'pull_right'  => __( 'Pull from Right', 'stax' ),
						'full_canvas' => __( 'Full Canvas', 'stax' ),
						'dropdown'    => __( 'Slide Down', 'stax' ),
					],
				],
				'conditional_header' => true,
				'transport'          => 'refresh',
	...
	private function add_columns_layout_controls( $row_setting_id, $row_id ) {
		...
		SettingsManager::get_instance()->add(
			[
				...
				'group'             => $row_setting_id,
				'tab'               => SettingsManager::TAB_LAYOUT,
				'label'             => __( 'Number of Columns', 'stax' ),
		...
		SettingsManager::get_instance()->add(
			[
				...
				'group'             => $row_setting_id,
				'tab'               => SettingsManager::TAB_LAYOUT,
				'label'             => __( 'Columns Layout', 'stax' ),
				'type'              => '\Stax\Customizer\Core\Controls\React\Builder_Columns',
				'section'           => $row_setting_id,
				'options'           => [
		...
		$align_choices = [
			...
			'center'     => [
				'tooltip' => __( 'Middle', 'stax' ),
				'icon'    => 'sort',
			],
			'flex-end'   => [
				'tooltip' => __( 'Bottom', 'stax' ),
				'icon'    => 'arrow-down',
			],
		];
		...
		SettingsManager::get_instance()->add(
			[
				...
				'default'               => 'flex-start',
				'section'               => $row_setting_id,
				'label'                 => __( 'Vertical Alignment', 'stax' ),
				'type'                  => '\Stax\Customizer\Core\Controls\React\Radio_Buttons',
				'live_refresh_selector' => true,
				'live_refresh_css_prop' => [
					'cssVar' => [
						'vars'     => '--vAlign',
						'selector' => '.' . $this->get_id() . '-' . $row_id . ' .stx-row',
					],
					'is_for' => 'vertical',
				],
				'options'               => [
	...
	private function add_new_builder_styles( $css_array, $row ) {
		...
		$css_array[] = [
			Dynamic_Selector::KEY_SELECTOR => '.' . $builder . '-' . $row . '-inner .stx-row',
			Dynamic_Selector::KEY_RULES    => [
